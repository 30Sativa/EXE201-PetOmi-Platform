...
import { Banknote, CalendarCheck, CreditCard, PackageSearch, Plus, QrCode, ReceiptText, RotateCcw, Trash2 } from "lucide-react"
...
import type { AppointmentListItemResponse, InvoiceResponse, PendingManualRefundItemResponse, SePayPaymentRequestResponse, SePayPaymentStatusResponse } from "@/types"
...
export default function ClinicBillingPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <DashboardSection title="Mở hóa đơn lịch hẹn" subtitle="Chọn lịch hẹn theo ngày để tạo hoặc mở hóa đơn. Nhân viên không cần tự nhớ mã kỹ thuật của lịch hẹn.">
        <div className="grid gap-3 xl:grid-cols-[170px_200px_minmax(0,1fr)]">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_160px_auto]">
          <AppointmentSelect
...
function AppointmentStatusSelect({
  ...
  return (
    <label className="grid min-w-0 gap-1.5 text-sm font-semibold text-po-text">
      Trạng thái
      <select
        value={value}
  ...
  return (
    <label className="grid min-w-0 gap-1.5 text-sm font-semibold text-po-text">
      ...
      <select
        ...
        onChange={(event) => onChange(event.target.value)}
        disabled={isLoading}
        className="h-11 min-w-0 rounded-2xl border border-po-border bg-white px-4 text-sm font-medium text-po-text outline-none transition focus:border-po-primary focus:ring-2 focus:ring-po-primary/20 disabled:opacity-60"
      >
        <option value="">
      ...
      <span className="flex items-center gap-1 text-xs font-medium text-po-text-subtle">
        <CalendarCheck className="size-3" />
        Dùng lịch đã check-in/hoàn tất để auto-compose dịch vụ và toa thuốc chính xác hơn.
      </span>
    </label>
  )
}
