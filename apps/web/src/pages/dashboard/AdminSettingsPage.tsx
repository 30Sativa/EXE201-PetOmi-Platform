                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {def?.type === "boolean" && (
                              <span className="inline-flex items-center rounded-full bg-po-surface-muted px-2 py-0.5 text-[10px] font-semibold text-po-text-muted">
                                Toggle
                              </span>
                            )}
                          </div>
                        ...
                        <div className="shrink-0">
                          {isEditing ? (
                            <div className="flex items-center gap-2">
                              {def?.type === "boolean" ? (
                                ...
                                <input
                                  ...
                                  value={editValue}
                                  onChange={(e) => setEditValue(e.target.value)}
                                  className="h-9 w-40 rounded-full bg-po-surface-muted/50 px-4 text-xs text-po-text ring-1 ring-po-border/60 focus:outline-none focus:ring-2 focus:ring-po-primary/40"
                                />
                              )}
                              ...
                              <button
                                ...
                                className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-po-success text-white transition hover:-translate-y-0.5 disabled:opacity-40"
                                title="Lưu"
                              >
                              ...
                              <button
                                onClick={cancelEdit}
                                disabled={updateMutation.isPending}
                                className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-po-surface-muted text-po-text-muted transition hover:-translate-y-0.5 disabled:opacity-40"
                                title="Hủy"
                              >
                                <X className="size-4" />
                              </button>
                            ...
                            <div className="flex items-center gap-2">
                              <span className="rounded-full bg-po-surface-muted/50 px-3 py-1 text-xs font-semibold text-po-text">
                                {setting.SettingValue}
                              </span>
                              <button
                                onClick={() => startEdit(setting)}
                                className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-po-primary-soft text-po-primary transition hover:-translate-y-0.5 hover:bg-po-primary hover:text-white"
                                title="Chỉnh sửa"
                              >
